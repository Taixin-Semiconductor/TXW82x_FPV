...
#define RC_WK_INIT_DEFAULT  137 //measured val 
#define RC_RF_INIT_DEFAULT  151 //measured each time
...
#define DSLEEP_DBG_PWMA15_EN        BIT(4)
...
#define GPIOD_REG_LEN           (0x0080/4 + 1) 
#define GPIOE_REG_LEN           (0x0080/4 + 1) 
...
struct dsleep_cfg{
    ...
    int32                       (*usr_hbdet_cb)(void *usr_hbdet_priv, uint8 *data, uint32 len);
    void                        *usr_hbdet_priv;
    int32                       (*usr_wkdet_cb)(void *usr_wkdet_priv, uint8 *data, uint32 len);
    void                        *usr_wkdet_priv;
    void                        (*dsleep_enter_hook)(void);
    int32                       (*dsleep_wakeup_hook)(uint8 wk_reason);
    ...
    uint32                      ip_addr;
    uint32                      gw_addr;
    uint32                      dhcp_renew_t1;//t0/2
    uint32                      user_gpioa_resv;
    uint32                      user_gpiob_resv;
    uint32                      user_gpioc_resv;
    uint32                      user_gpiod_resv;
    uint32                      user_gpioe_resv;
    uint32                      user_gpiof_resv;
    uint32                      gpioa_regs[GPIOA_REG_LEN];
    uint32                      gpiob_regs[GPIOB_REG_LEN];
    uint32                      gpioc_regs[GPIOC_REG_LEN];
    uint32                      gpiod_regs[GPIOD_REG_LEN];
    uint32                      gpioe_regs[GPIOE_REG_LEN];
    uint32                      gpiof_regs[GPIOE_REG_LEN];
    uint32                      pmu_regs[0x100/4];
    uint32                      sys_wdt_ms;
    uint32                      lp_wdt_ms;
    uint32                      rtc_wkio;
    int32                       (*tx_hdl_cb)(void *data, uint32 len);
};
...
struct dsleep_priv {
    ...
    struct hguart_v2        *dev_uart0;
    struct hguart_v2        *dev_uart1;
    ...
    //add from other .h
    struct sk_buff                      *null_psm;//todo: del alloc
    ...
    uint16                              skb_null_psm_used   : 1,//tbd: del
    ...
    //move from dcfg
    uint8                      tx_buff[22 + 256];//22:lmac_bgn_hw_frm_info
    ...
    uint32                      sleep_lead;
    uint64                      tmrao_bkn;
    uint64                      tmrao_sleep;//开始休眠的时间
    uint64                      tmrao_rxstart;
    uint64                      tmrao_max_idle;
    uint64                      tmrao_ip_lease;//用于记录最后一次的dhcp
    uint64                      tmrao_wkup;//用于设定休眠的唤醒时间
    
    uint64                      timestamp_pre;//beacon 包中携带的时间戳
    int32                       bkn_delay_pre;
    int32                       ts_op_avg;
    uint32                      bkto;
    struct dsleep_arp_hdr       *arp_req;
    uint8                       arp_req_sender_mac[6];
    uint8                       arp_req_sender_ip[4];

    /* 管理所有管理帧的sn。只确保递增，不确保连续 */
    //uint16                      tx_mgmt_sn : 12; // nonos有管理mgmt_sn，这里不用保存了
    uint8                       addba_req_token;
    uint8                       addba_req_tid; 
