...
typedef enum {
    ...
    DSLEEP_WK_REASON_DHCP_RENEW          = 34,
    DSLEEP_WK_REASON_UNKNOWN             = 255,
} DSLEEP_WK_REASON;
...
struct sys_dsleep_boot{
    union {
        struct {
            uint32              crc_bypass  :   1,
                                crc_len     :   30;
            uint32              wakeup_handler;
            uint32              wakeup1_handler;
            uint32             *dsleep_priv;
            uint32             *lmac_ops;
            volatile uint32    cpu1_sleeped :   1;
        } param;
        uint32                  rsvd[32];
    };
};
...
enum DSLEEP_IOCTL_CMD {
    ...
    DSLEEP_IOCTL_SET_DSLEEP_SENS_LEVEL,     //discard
    DSLEEP_IOCTL_SET_DSLEEP_PWM_SEL,
    DSLEEP_IOCTL_SET_DSLEEP_PWM_MODE,       //10
    ...
    DSLEEP_IOCTL_SET_FAKE_TIM_FADEOUT,      //20
    ...
    DSLEEP_IOCTL_SET_ROUTER,
    DSLEEP_IOCTL_SET_GPIOD_RESV,
    DSLEEP_IOCTL_SET_GPIOE_RESV,
    DSLEEP_IOCTL_SET_GPIOF_RESV,            //30
    DSLEEP_IOCTL_SET_MAX_IDLE_RATIO,
    DSLEEP_IOCTL_SET_NTP_UPDATE,
    DSLEEP_IOCTL_SET_ENTER_HOOK,
    DSLEEP_IOCTL_SET_WAKEUP_HOOK,
...
//ioctrl: set_cfg
#define dsleep_set_usr_hbdet_cb(priv, func)         dsleep_ioctl(DSLEEP_IOCTL_SET_USR_HBDET_CB, (uint32)priv, (uint32)(func))
...
#define dsleep_set_router(gw_addr, t1_renew)        dsleep_ioctl(DSLEEP_IOCTL_SET_ROUTER, (uint32)gw_addr, (uint32)t1_renew)
#define dsleep_set_user_giod(user_giod)             dsleep_ioctl(DSLEEP_IOCTL_SET_GPIOD_RESV, (uint32)user_giod, 0)
#define dsleep_set_user_gioe(user_gioe)             dsleep_ioctl(DSLEEP_IOCTL_SET_GPIOE_RESV, (uint32)user_gioe, 0)
#define dsleep_set_user_giof(user_giof)             dsleep_ioctl(DSLEEP_IOCTL_SET_GPIOF_RESV, (uint32)user_giof, 0)
#define dsleep_set_max_idle_ratio(numerator, denominator)   dsleep_ioctl(DSLEEP_IOCTL_SET_MAX_IDLE_RATIO, (uint8)(numerator), (uint8)(denominator))
#define dsleep_set_ntp_update()                     dsleep_ioctl(DSLEEP_IOCTL_SET_NTP_UPDATE, 0, 0)
#define dsleep_set_enter_hook(func)                 dsleep_ioctl(DSLEEP_IOCTL_SET_ENTER_HOOK, (uint32)func, 0)
#define dsleep_set_wakeup_hook(func)                dsleep_ioctl(DSLEEP_IOCTL_SET_WAKEUP_HOOK, (uint32)func, 0)
