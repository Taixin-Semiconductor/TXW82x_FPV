...
{
    // SYS PWR EN
    gpio_set_dir(MACRO_PIN(PIN_SYS_PWR_EN), GPIO_DIR_OUTPUT);
    gpio_set_val(MACRO_PIN(PIN_SYS_PWR_EN), 1);

    // VDD PWR CTRL
    // USB_DET/VDD_CTL= 1      USB_DET/VDD_CTL= 0/Floating
    // NORMAL MODE:VDD=1.2V    LOW PWR MODE:VDD=1.05V
 ...
 ************************************************/
static void app_takephoto_init(void)
{
...
// 函数入口先确定方案,将需要的模块attatch
void app_battery_camera_1080p_demo_init(void)
{
    int ret = RET_OK;
    app_print_init();
    app_heap_init();
    ...
    if (app_1080p_lowpower_register() != RET_OK) {
        os_printf("%s:%d register lowpower modules failed\n",
