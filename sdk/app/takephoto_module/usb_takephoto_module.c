...
void usb_takephoto_init(const char *source_msi_name, const char *photo_msi_name, const char *thumb_msi_name)
{
    ...
    {
        // 给到缩略图模块去解码后编码小缩略图
        msi_add_output(usb_jpg_thumb_msi, NULL, thumb_msi_name);
    }
    if (usb_jpg_thumb_msi)
    {
        // 文件保存的msi
        msi_add_output(usb_jpg_thumb_msi, NULL, R_FILE_MSI);
    }
}
...
static void usb_takephoto_thumb_init(const char *thumb_msi_name)
{
    ...
    {
        // 给到gen420去重新生成缩略图jpg
        msi_add_output(decode_msi, NULL, R_GEN420_THUMB_JPG_USB);
    }
    ...
    {
        // 处理原图,然后给到解码后生成缩略图
        msi_add_output(thumb_msi, NULL, R_THUMB_DECODE_MSG_USB);
