...
uint32 net_h264_sema_down(int32 tmo_ms)
{
	...
	retdata = os_msgq_get2(&net_h264_msg, tmo_ms, &retval);
	if(retval == 0){
		return retdata;
	}else{
		return retval;
...
void udp_handle_server_status_read_workqueue(void *ei, uint32_t *status_fd){	
	int retval;
	int ret;
	uint8_t itk;
	...
	if(server_frame[id].lost_num != 0){
		BABY_DBG("L(%d)",server_frame[id].lost_num);
	}else{
		BABY_DBG("R%d",id);
	}
...
void udp_handle_server_status_thread(void *d){	
	...
	dev_tbl->udp_read_status_ev = eloop_add_fd( handle_protocol_fd, EVENT_READ, EVENT_F_ENABLED, udp_handle_server_status_read_workqueue, &handle_protocol_fd );
...
void udp_handle_server_data_thread(uint32_t *d){
	...
	while(1){
		...
		ret = recvfrom (handle_data_protocol_fd, photo_buf, MAX_VIDEO_PKT_LEN+sizeof(data_head), 0, &remote_addr, (socklen_t*)&retval);
		id = 0;
		for(itk = 0;itk < 10;itk++){
			...
			for(itk = 0;itk < 3;itk++){
				if(framecnt[itk] != 0){						         //当前id存在帧
					...
					if(flush_video){
						...
						sys_dcache_clean_range(decmsg[decframe[itk]].addr,decmsg[decframe[itk]].len+4);
						...
						{	
							...
							if((oldw[decmsg[decframe[itk]].devid] != server_resolution[decmsg[decframe[itk]].devid].target_width) || (oldh[decmsg[decframe[itk]].devid] != server_resolution[decmsg[decframe[itk]].devid].target_high)){
								...
								if(decmsg[decframe[itk]].devid == 1) {
									scale2_output_size_local_change(decmsg[decframe[itk]].devid,0,0,0,320,240);		
								}	
								else {
									scale2_output_size_local_change(decmsg[decframe[itk]].devid,0,320,0,320,240);	     
								}
...
void user_tcpServerAccept(void *e, void *d)    //屏端
{
	...
	if(mask == 0){													    //当前IP没被分配
		devtab[devnum].ipaddr = client.sin_addr.s_addr;
		devtab[devnum].tcpfd  = tcp_fd;
		devnum++;														//标明下个设备号
	}
	
	eloop_add_fd( tcp_fd, EVENT_READ, EVENT_F_ENABLED, tcp_handle_server, tcp_fd );
}
