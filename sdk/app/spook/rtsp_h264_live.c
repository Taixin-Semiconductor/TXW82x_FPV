...
static void self_creat(struct rtsp_source *source, void *priv)
{
    ...
    if (source->priv)
    {
        ...
        if (memcmp(path, "/loop/RECA/", strlen("/loop/RECA/")) == 0)
        {
            ...
            if (r->video_msi && audio_dev)
            {
                r->a_msi = rtsp_audio_msi_init(R_RTP_AUDIO);
                msi_add_output(r->video_msi, NULL, NULL, R_RTP_AUDIO);
            }
        }
        else
        {
            ...
            if(audio_dev)
            {
                ...
                if (r->audio_msi)
                {
                    ...
                    msi_add_output(r->audio_msi, NULL, NULL, R_RTP_AUDIO);
                }
            }
        ...
        if (r->video_msi)
        {
            ...
            if (r->v_msi)
            {
                msi_add_output(r->video_msi, NULL, NULL, R_RTP_JPEG_H264); // 将video_msi的数据输出到R_RTP_JPEG_H264的msi,即v_msi
...
static void self_destory(struct rtsp_source *source)
{
    ...
    if (r)
    {
        if (r->video_msi)
        {
            msi_del_output(r->video_msi, NULL, NULL, R_RTP_JPEG_H264);
        ...
        if (r->audio_msi)
        {
            msi_del_output(r->audio_msi, NULL, NULL, R_RTP_AUDIO);
            msi_put(r->audio_msi);
        }
...
extern void *h264_encode_init(const char *encode_name);
extern void h264_encode_deinit(void *d);
...
static int self_set_track(void *d)
{
	...
	if(rtsp_end_block(source) != 0)
	{
        ...
        {
            h264_encode_init((char*)source->rtp_name->video_encode_name);
        }
		set_video_h264_track((char*)source->rtp_name->video_encode_name, source);
		...
		register_live_fn(source, self_creat, self_destory, self_play, self_track_deinit);
		return rtsp_end_block(source);
	}
	return 0;
}

void rtsp_h264_set_path(const rtp_name *rtsp)
{
...
void rtsp_h264_live_init(const rtp_name *rtsp)
{
    ...
    register_live_fn(source, self_creat, self_destory, self_play, NULL);
