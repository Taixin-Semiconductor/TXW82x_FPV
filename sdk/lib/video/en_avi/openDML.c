...
int OMDLvideo_header_write(void *f_write, void *f, AVI_INFO *msg, ODMLAVIFILEHEADER *headerbuf)
{
	...
	aviheader->avih.dwMicroSecPerFrame = (1000 * 1000 / msg->frame_rate);
	...
	aviheader->avih.dwWidth = msg->win_w;
	aviheader->avih.dwHeight = msg->win_h;
	...
	aviheader->strh_v.dwRate = msg->frame_rate;
	aviheader->strh_v.dwStart = 0;
	aviheader->strh_v.rcFrame.right = msg->win_w;
	aviheader->strh_v.rcFrame.bottom = msg->win_h;
	...
	aviheader->bitmapinfo.biWidth = msg->win_w;
	aviheader->bitmapinfo.biHeight = msg->win_h;
	...
	aviheader->vprp.dwVerticalRefreshRate = msg->frame_rate;
...
int insert_frame(ODMLBUFF *buff, void *pavi, float *tim_diff)
{
	...
	int insert_num = 1;
	if (buff->cur_timestamp == 0 || 0 == buff->last_timestamp)
		return 0;
	// 增加时间差,后面还需要减去一帧的时间差
