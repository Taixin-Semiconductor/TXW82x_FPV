...
/* ------------ TXSEMI Acceleration module ------------ */

#ifdef TXW82X
/* 加速模块仅支持sram，需要将lv mem打到sram配合使用 */
...
typedef struct {
    ...
    __IO uint32_t MASK_ADDR;
    __IO uint32_t STRIDE0;
    __IO uint32_t STRIDE1;
    __IO uint32_t OPA_CONF;
    __IO uint32_t FG_COLOR_SAVE;
    __IO uint32_t BG_COLOR_SAVE;
    __IO uint32_t RES_COLOR_SAVE;
    __IO uint32_t OPA_SAVE;
    __IO uint32_t LST_DEST_COLOR;
    __IO uint32_t LST_SRC_COLOR;
    __IO uint32_t LST_RES_COLOR;
    __IO uint32_t LST_OPA;
} FILL_TypeDef;
#define FILL                    ((FILL_TypeDef   *)FILL_BASE)
...
typedef struct {
    ...
    __IO uint32_t PARA1;
    __IO uint32_t PARA2;
    __IO uint32_t MASK_PARAM0;
    __IO uint32_t MASK_PARAM1;
    __IO uint32_t MASK_PARAM2;
    __IO uint32_t MASK_PARAM3;
    __IO uint32_t MASK_LINE;
    __IO uint32_t SH_BUF;
    __IO uint32_t OPA_START_ON_Y;
    __IO uint32_t X_START_ON_Y;
    __IO uint32_t CIR_OPA;
    __IO uint32_t SH_UPS_BLUR_BUF;
	__IO uint32_t STA;
} SHADOW_TypeDef;
#define SHADOW                  ((SHADOW_TypeDef   *)SHADOW_BASE)

#define TX_LV_ROTATE_HARDWARE_EN        0
#define TX_LV_FILL_HARDWARE_EN          0
#define TX_LV_SHADOW_HARDWARE_EN        0

#endif

#ifndef TX_LV_ROTATE_HARDWARE_EN
#define TX_LV_ROTATE_HARDWARE_EN        0
#endif

#ifndef TX_LV_FILL_HARDWARE_EN
#define TX_LV_FILL_HARDWARE_EN          0
#endif

#ifndef TX_LV_SHADOW_HARDWARE_EN
#define TX_LV_SHADOW_HARDWARE_EN        0
#endif
/* ---------------------------------------------------- */
...
#define LV_USE_BUILTIN_MALLOC 0
    ...
    #define LV_MEM_SIZE (48U * 1024U)//(128U * 1024U)          /*[bytes]*/
...
#define LV_MALLOC       hg_lv_mem_malloc   
#define LV_REALLOC      hg_lv_mem_realloc
#define LV_FREE         hg_lv_mem_free
#define LV_MEMSET       hg_lv_mem_memset
#define LV_MEMCPY       hg_lv_mem_memcpy
