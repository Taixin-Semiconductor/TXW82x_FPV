...
static void sd_log_card_status_after_timeout(struct sdh_device *host, const char *operation)
{
    uint32 status = 0;
    int ret = sd_get_card_status(host, &status);
    if (ret == RET_OK) {
        SDHC_ERR_PRINTF("sd %s timeout, CMD13 status:0x%08x state:%u\r\n",
                        operation, status, (status >> 9) & 0xf);
    } else {
        SDHC_ERR_PRINTF("sd %s timeout, CMD13 failed:%d status:0x%08x\r\n",
                        operation, ret, status);
    }
}
...
//                    host->sd_write_retry_flag = 1;
    ...
    if (ret == MMCSD_CMP_TIMEOUT) {
        sd_log_card_status_after_timeout(host, "write");
    }
...
//                    host->sd_write_retry_flag = 1;
    ...
    if (ret == MMCSD_CMP_TIMEOUT) {
        sd_log_card_status_after_timeout(host, "read");
    }
...
uint8 sd_udisk_get_status(void *dev) {
	return ((struct sdh_device *)dev)->sd_opt == SD_OFF;
}

int sd_udisk_read(void *dev, uint32_t lba, uint32_t block_count, uint8* buf) {
    return sd_multiple_read((struct sdh_device *)dev, lba, SECTOR_SIZE * block_count,buf);
}

int sd_udisk_write(void *dev, uint32_t lba, uint32_t block_count, uint8* buf) {
    return sd_multiple_write((struct sdh_device *)dev, lba, SECTOR_SIZE * block_count,buf);
}
...
int32 sdh_loop(struct os_work *work)
{
    ...
    {    
        int32 err = sd_init(sdh, sdhost_wk.clk, sdhost_wk.flags);
        if(err){
            dev_hotplug_out(HG_SD0_DEVID);
            sdh->sd_opt = SD_OFF;
        }else{
            os_printf(KERN_NOTICE"SD card detect!\r\n");
...
uint32 sdhost_init(uint32 clk, uint32 flags)
{
    ...
    sdhost_wk.clk = clk;
    sdhost_wk.flags = flags;
    if(sdh){
        err = sd_init(sdh, sdhost_wk.clk, sdhost_wk.flags);
        if(sdhost_wk.wk.init == 0  && sdhost_wk.wk.running == 0){
            sdhost_wk.host = sdh;
            sdhost_wk.isregister = 1;
            OS_WORK_INIT(&sdhost_wk.wk, sdh_loop, 0);
