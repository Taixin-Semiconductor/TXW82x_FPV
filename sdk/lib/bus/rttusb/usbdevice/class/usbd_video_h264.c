...
#undef  R_USBD_VIDEO
#define R_USBD_VIDEO        R_USBD_VIDEO_H264
...
#define VIDEO_H26x_FORMAT_INDEX 1
#else
#define VIDEO_H26x_HEADER_LEN 0
#define VIDEO_H26x_FORMAT_INDEX 0
#endif

#define VIDEO_VS_INPUT_HEADER_wTotalLength   (VIDEO_VS_INPUT_HEADER_DESC_LEN + VIDEO_H26x_HEADER_LEN)

#define VIDEO_VS_INPUT_HEADER_bNumFormats    1
...
static struct uvc_vs_noep_descriptor vs_noep_desc = 
{
    ...
    /* VS H264 Format Descriptor */
    {
        ...
        VIDEO_H26x_FORMAT_INDEX,                                                    //bFormatIndex
...
static rt_uint32_t usbd_video_h264_payload_header_fill(rt_uint8_t *input, rt_uint32_t input_len, rt_uint8_t *output, rt_uint8_t *uvc_header, rt_uint8_t uvc_header_len)
{
    ...
    if(input_len < VIDEO_PACKET_SIZE - uvc_header_len)       //2为header的长度
    {
        ...
        hw_memcpy(&output[2], input, input_len);
...
static int32_t usbd_h264_stream_action(struct msi *msi, uint32_t cmd_id, uint32_t param1, uint32_t param2)
{
    int32_t ret = RT_EOK;
    switch (cmd_id)
    {
        ...
        case MSI_CMD_TRANS_FB:
        {
            ...
            if (fb->mtype != F_H264 || fb->stype != FSTYPE_H264_VPP_DATA0)
            {
                ret = RT_ERROR;
            }
        }
...
static void usbd_uvc_entry(void *parameter)
{
    ...
    s->action = usbd_h264_stream_action;
            ...
            if(get_f)
            {
                ...
                while(flen)
                {
                    ...
                    if (e & EVENT_VIDEO_DATA)
                    {
                        ...
                        {
                            out_len = usbd_video_h264_payload_header_fill((rt_uint8_t *)(frame_buf_addr + index), (VIDEO_PACKET_SIZE-sizeof(uvc_header)), usbd_uvc.buffer, (rt_uint8_t*)&uvc_header,sizeof(uvc_header));
                            ...
                            out_len = usbd_video_h264_payload_header_fill((rt_uint8_t *)(frame_buf_addr + index), flen, usbd_uvc.buffer, (rt_uint8_t*)&uvc_header,sizeof(uvc_header));
...
static rt_bool_t usbd_uvc_init(struct ufunction *func)
{
    ...
    usbd_uvc_thread = rt_thread_create("usbd_uvc_thread_h264",
                                        usbd_uvc_entry, func,
                                        1024,
