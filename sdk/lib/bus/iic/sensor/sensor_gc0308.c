#include "sys_config.h"
#include "typesdef.h"
#include "lib/video/dvp/cmos_sensor/csi.h"
#include "tx_platform.h"
#include "list.h"
#include "dev.h"
#include "hal/i2c.h"


#if DEV_SENSOR_GC0308

SENSOR_INIT_SECTION static const unsigned char gc0308_640x480_raw8_reg[CMOS_INIT_LEN]=
{
	0xfe,0x80,			
	0xfe,0x00,//setpage0
	0xd2,0x10,//closeAEC
	0x22,0x55,//closeAWB
	0x03,0x01,
	0x04,0x2c,
	0x5a,0x56,
	0x5b,0x40,
	0x5c,0x4a,			
	0x01,0x6a,
	0x02,0x1a,
	0x0f,0x00,
	0x05,0x00,//window
	0x06,0x00,
	0x07,0x00,
	0x08,0x00,
	0x09,0x01,
	0x0a,0xe8,
	0x0b,0x02,
	0x0c,0x88,
	0x46,0x80,//crop
	0x47,0x00,	
	0x48,0x00,
	0x49,0x01,	
	0x4a,0xe0,
	0x4b,0x02,	
	0x4c,0x80,
	0x0d,0x02,
	0x0e,0x02,
	0x10,0x22,
	0x11,0xfd,
	0x12,0x2a,
	0x13,0x00,
	0x15,0x0a,
	0x16,0x05,
	0x17,0x01,
	0x18,0x44,
	0x19,0x44,
	0x1a,0x1e,
	0x1b,0x00,
	0x1c,0xc1,
	0x1d,0x08,
	0x1e,0x60,
	0x1f,0x16,
	0x20,0xff,
	0x21,0xf8,
	0x22,0x57,
	0x24,0xa2,
	0x25,0x0f,
	//outputsync_mode
	0x26,0x03,
	0x2f,0x01,
	0x30,0xf7,
	0x31,0x50,
	0x32,0x00,
	0x39,0x04,
	0x3a,0x18,
	0x3b,0x20,
	0x3c,0x00,
	0x3d,0x00,
	0x3e,0x00,
	0x3f,0x00,
	0x50,0x10,
	0x53,0x82,
	0x54,0x80,
	0x55,0x80,
	0x56,0x82,
	//RegistersofPage0
	0xfe,0x00,
	0x10,0x26,
	0x11,0x0d,
	0x1a,0x2a,
	0x1c,0x49,
	0x1d,0x9a,
	0x1e,0x61,
	0x3a,0x20,
	0x50,0x14,
	0x53,0x80,
	0x56,0x80,
	0x14,0x10,	
	0x24,0xb8,//outputrawdata	
	0x20,0x00,
	0x21,0x00,
	0x22,0x00,
	0xff,0xff,
	/*GC0308_H_V_Switch,
	1://normal
		0x14,0x10,
	2://IMAGE_H_MIRROR
		0x14,0x11,
	3://IMAGE_V_MIRROR
		0x14,0x12,
	4://IMAGE_HV_MIRROR
		0x14,0x13,
	*/		
	
};

SENSOR_INIT_SECTION static const unsigned char gc0308_640x480_yuv422_reg[CMOS_INIT_LEN]=
{
	,
//	0x14,0x12,
	0x14,0x10,

	0xff,0xff,
};

extern struct i2c_device *iic_test;

static void GC0308_rotate(uint32 r)
{
	int8 buf[2];
	buf[0] = 0x14;		//0',180'
	buf[1] = 0x10|(r<<0);
	//Sensor_WriteRegister(SENSOR_IIC,buf,1,1);
	//i2c_sensor_write(iic_test,buf,1,1,0x42,0x43);
	i2c_write(iic_test,buf,1,&buf[1],1);
	

}



uint32 GC0308_hvblank(int8 dh,int8 dv)
{
#define HBLANK_BASE	0x6aUL
#define VBLANK_BASE	0x12UL
	uint32 i;
	int32 h = (int32)HBLANK_BASE + dh; if(h < 0) h = 0;
	int32 v = (int32)VBLANK_BASE + dv; if(v < 0) v = 0;
	const uint8 t[][2] = {
		{0x01,h & 0xff},//0x6a//0xde //HB[7:0]
		{0x02,v & 0xff},  //0x0c //VB[7:0]
		{0x0f,((v >> 8)<<4)|(h >> 8)} //{0x0f,0x00}, //VB/HB [11:8]
	};
	
	//for(i=0;i<3;i++)
		//Sensor_WriteRegister(SENSOR_IIC,(uint8 *)&t[i][0],1,1);
	for(i=0;i<3;i++)	
		//i2c_sensor_write(iic_test,(uint8 *)&t[i][0],1,1,0x42,0x43);
		i2c_write(iic_test,(int8 *)&t[i][0],1,(int8 *)&t[i][1],1);

	return (h<<16)|v;
}


static const SensorWorkMode gc0308_supported_modes[] = {
#if 0
    {
        .mode = SENSOR_TYPE_MASTER,
		.bayer_patten = ISP_BAYER_FORMAT_GBRG,
        .width = 640,
        .height = 480,
		.dvp = {
            .hsync_pol=1,
            .vsync_pol=1,
 		},
        .fps_table = {
            {.fps = 25, .vts = 1350},
            {.fps = 15, .vts = 2253},
        },
        .reg_list = gc0308_640x480_raw8_reg,
    },
#endif
    {
        .mode = SENSOR_TYPE_MASTER,
        .width = 640,
        .height = 480,
		.dvp = {
            .hsync_pol=1,
            .vsync_pol=1,
		},
        .fps_table = {
            {.fps = 25, .vts = 1350},
            {.fps = 15, .vts = 2253},
        },
        .reg_list = gc0308_640x480_yuv422_reg,
    },



};


SENSOR_OP_SECTION const _Sensor_Adpt_ gc0308_cmd = 
{
    .supported_modes = (SensorWorkMode*)gc0308_supported_modes,
    .mode_num        = ARRAY_SIZE(gc0308_supported_modes),

    .vts_reg = {0x41,0x42},
    .vts_reg_num = 2,
    .sensor_iic = {
        0x9b,0x42,0x43,0x01,0x01,0x00
    },
    .sensor_isp_cfg = {
        .mirror       = 0,
        .reverse      = 0,
        .expo_max_gain = 110<<8,
        .min_frame_vb = 8,
        .expo_update_delay = 2,
        
        .input_format = ISP_INPUT_DAT_FORMAT_YUV422,
//        .adjust_func  = (isp_ae_func     )GC2053_ae_adjust,
//        .img_opt      = (sensor_img_opt  )gc2053_img_opt,
//        .fps_opt      = (sensor_fps_opt  )gc2053_fps_opt,
        // .isp_iq_param   = (_Sensor_ISP_Init*)&2053_isp_param_init,
    },
};

#endif

