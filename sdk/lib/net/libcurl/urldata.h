...
struct ssl_primary_config {
  ...
  _BIT_(verifypeer);       /* set TRUE if this is desired */
  _BIT_(verifyhost);       /* set TRUE if CN/SAN must match hostname */
  _BIT_(verifystatus);     /* set TRUE if certificate status must be checked */
  _BIT_(cache_session);    /* cache session or not */
};
...
struct ssl_config_data {
  ...
  _BIT_(certinfo);     /* gather lots of certificate info */
  _BIT_(earlydata);    /* use tls1.3 early data */
  _BIT_(enable_beast); /* allow this flaw for interoperability's sake */
  _BIT_(no_revoke);    /* disable SSL certificate revocation checks */
  _BIT_(no_partialchain); /* do not accept partial certificate chains */
  _BIT_(revoke_best_effort); /* ignore SSL revocation offline/missing revocation
                              list errors */
  _BIT_(native_ca_store); /* use the native ca store of operating system */
  _BIT_(auto_client_cert);   /* automatically locate and use a client
                              certificate for authentication (Schannel) */
  _BIT_(custom_cafile); /* application has set custom CA file */
  _BIT_(custom_capath); /* application has set custom CA path */
  _BIT_(custom_cablob); /* application has set custom CA blob */
};
...
struct digestdata {
  ...
  int nc; /* nonce count */
  unsigned char algo;
  _BIT_(stale); /* set true for re-negotiation */
  _BIT_(userhash);
#endif
};
#endif
...
struct ConnectBits {
  ...
  _BIT_(httpproxy);  /* if set, this transfer is done through an HTTP proxy */
  _BIT_(socksproxy); /* if set, this transfer is done through a socks proxy */
  _BIT_(proxy_user_passwd); /* user+password for the proxy? */
  _BIT_(tunnel_proxy);  /* if CONNECT is used to "tunnel" through the proxy.
                         This is implicit when SSL-protocols are used through
                         proxies, but can also be enabled explicitly by
                         apps */
  _BIT_(proxy); /* if set, this transfer is done through a proxy - any type */
#endif
  /* always modify bits.close with the connclose() and connkeep() macros! */
  _BIT_(close); /* if set, we close the connection after this request */
  _BIT_(reuse); /* if set, this is a reused connection */
  _BIT_(altused); /* this is an alt-svc "redirect" */
  _BIT_(conn_to_host); /* if set, this connection has a "connect to host"
                        that overrides the host in the URL */
  _BIT_(conn_to_port); /* if set, this connection has a "connect to port"
                        that overrides the port in the URL (remote port) */
  _BIT_(ipv6_ip); /* we communicate with a remote site specified with pure IPv6
                   IP address */
  _BIT_(ipv6);    /* we communicate with a site using an IPv6 address */
  _BIT_(do_more); /* this is set TRUE if the ->curl_do_more() function is
                   supposed to be called, after ->curl_do() */
  _BIT_(protoconnstart);/* the protocol layer has STARTED its operation after
                         the TCP layer connect */
  _BIT_(retry);         /* this connection is about to get closed and then
                         re-attempted at another connection. */
#ifndef CURL_DISABLE_FTP
  _BIT_(ftp_use_epsv);  /* As set with CURLOPT_FTP_USE_EPSV, but if we find out
                         EPSV does not work we disable it for the forthcoming
                         requests */
  _BIT_(ftp_use_eprt);  /* As set with CURLOPT_FTP_USE_EPRT, but if we find out
                         EPRT does not work we disable it for the forthcoming
                         requests */
  _BIT_(ftp_use_data_ssl); /* Enabled SSL for the data connection */
  _BIT_(ftp_use_control_ssl); /* Enabled SSL for the control connection */
#endif
#ifndef CURL_DISABLE_NETRC
  _BIT_(netrc);         /* name+password provided by netrc */
#endif
  _BIT_(bound); /* set true if bind() has already been done on this socket/
                 connection */
  _BIT_(upgrade_in_progress); /* protocol upgrade is in progress */
  _BIT_(multiplex); /* connection is multiplexed */
  _BIT_(tcp_fastopen); /* use TCP Fast Open */
  _BIT_(tls_enable_alpn); /* TLS ALPN extension? */
#ifndef CURL_DISABLE_DOH
  _BIT_(doh);
#endif
#ifdef USE_UNIX_SOCKETS
  _BIT_(abstract_unix_socket);
#endif
  _BIT_(sock_accepted); /* TRUE if the SECONDARYSOCKET was created with
                         accept() */
  _BIT_(parallel_connect); /* set TRUE when a parallel connect attempt has
                            started (happy eyeballs) */
  _BIT_(aborted); /* connection was aborted, e.g. in unclean state */
  _BIT_(no_reuse); /* connection should not be reused */
  _BIT_(shutdown_handler); /* connection shutdown: handler shut down */
  _BIT_(shutdown_filters); /* connection shutdown: filters shut down */
  _BIT_(in_cpool);     /* connection is kept in a connection pool */
};
...
struct PureInfo {
  ...
  _BIT_(timecond);  /* set to TRUE if the time condition did not match, which
                     thus made the document NOT get fetched */
  _BIT_(used_proxy); /* the transfer used a proxy */
};
...
#define CURR_TIME (5 + 1) /* 6 entries for 5 seconds */
  ...
  _BIT_(ul_size_known);
  _BIT_(dl_size_known);
  _BIT_(headers_out); /* when the headers have been written */
  _BIT_(callback);  /* set when progress callback is used */
  _BIT_(is_t_startransfer_set);
};
...
struct auth {
  ...
  unsigned long avail; /* Bitmask for what the server reports to support for
                          this resource */
  _BIT_(done);  /* TRUE when the auth phase is done and ready to do the
                 actual request */
  _BIT_(multipass); /* TRUE if this is not yet authenticated but within the
                     auth multipass negotiation */
  _BIT_(iestyle); /* TRUE if digest should be done IE-style or FALSE if it
...
struct UrlState {
  ...
  _BIT_(multi_owned_by_easy);

  _BIT_(this_is_a_follow); /* this is a followed Location: request */
  _BIT_(refused_stream); /* this was refused, try again */
  _BIT_(errorbuf); /* Set to TRUE if the error buffer is already filled in.
                    This must be set to FALSE every time _easy_perform() is
                    called. */
  _BIT_(allow_port); /* Is set.use_port allowed to take effect or not. This
                      is always set TRUE when curl_easy_perform() is called. */
  _BIT_(authproblem); /* TRUE if there is some problem authenticating */
  /* set after initial USER failure, to prevent an authentication loop */
  _BIT_(wildcardmatch); /* enable wildcard matching */
  _BIT_(disableexpect);    /* TRUE if Expect: is disabled due to a previous
                            417 response */
  _BIT_(use_range);
  _BIT_(rangestringalloc); /* the range string is malloc()'ed */
  _BIT_(done); /* set to FALSE when Curl_init_do() is called and set to TRUE
                when multi_done() is called, to prevent multi_done() to get
                invoked twice when the multi interface is used. */
#ifndef CURL_DISABLE_COOKIES
  _BIT_(cookie_engine);
#endif
  _BIT_(prefer_ascii);   /* ASCII rather than binary */
#ifdef CURL_LIST_ONLY_PROTOCOL
  _BIT_(list_only);      /* list directory contents */
#endif
  _BIT_(url_alloc);   /* URL string is malloc()'ed */
  _BIT_(referer_alloc); /* referer string is malloc()ed */
  _BIT_(wildcard_resolve); /* Set to true if any resolve change is a wildcard */
  _BIT_(upload);         /* upload request */
  _BIT_(internal); /* internal: true if this easy handle was created for
                    internal use and the user does not have ownership of the
                    handle. */
  _BIT_(http_ignorecustom); /* ignore custom method from now */
#ifndef CURL_DISABLE_HTTP
  _BIT_(http_hd_te); /* Added HTTP header TE: */
  _BIT_(http_hd_upgrade); /* Added HTTP header Upgrade: */
  _BIT_(http_hd_h2_settings); /* Added HTTP header H2Settings: */
#endif
};
...
struct UserDefined {
  ...
  */
  ...
  _BIT_(connect_only); /* make connection/request, then let application use the
                        socket */
  _BIT_(connect_only_ws); /* special websocket connect-only level */
#ifndef CURL_DISABLE_SMTP
  _BIT_(mail_rcpt_allowfails); /* allow RCPT TO command to fail for some
                                recipients */
#endif
#ifndef CURL_DISABLE_MIME
  _BIT_(mime_formescape);
#endif
  _BIT_(is_fread_set); /* has read callback been set to non-NULL? */
#ifndef CURL_DISABLE_TFTP
  _BIT_(tftp_no_options); /* do not send TFTP options requests */
#endif
  _BIT_(sep_headers);     /* handle host and proxy headers separately */
#ifndef CURL_DISABLE_COOKIES
  _BIT_(cookiesession);   /* new cookie session? */
#endif
  _BIT_(crlf);            /* convert crlf on ftp upload(?) */
#ifdef USE_SSH
  _BIT_(ssh_compression);            /* enable SSH compression */
#endif

/* Here follows boolean settings that define how to behave during
   this session. They are STATIC, set by libcurl users or at least initially
   and they do not change during operations. */
  _BIT_(quick_exit);       /* set 1L when it is okay to leak things (like
                            threads), as we are about to exit() anyway and
                            do not want lengthy cleanups to delay termination,
                            e.g. after a DNS timeout */
  _BIT_(get_filetime);     /* get the time and get of the remote file */
#ifndef CURL_DISABLE_PROXY
  _BIT_(tunnel_thru_httpproxy); /* use CONNECT through an HTTP proxy */
#endif
  _BIT_(prefer_ascii);     /* ASCII rather than binary */
  _BIT_(remote_append);    /* append, not overwrite, on upload */
#ifdef CURL_LIST_ONLY_PROTOCOL
  _BIT_(list_only);        /* list directory */
#endif
#ifndef CURL_DISABLE_FTP
  _BIT_(ftp_use_port);     /* use the FTP PORT command */
  _BIT_(ftp_use_epsv);     /* if EPSV is to be attempted or not */
  _BIT_(ftp_use_eprt);     /* if EPRT is to be attempted or not */
  _BIT_(ftp_use_pret);     /* if PRET is to be used before PASV or not */
  _BIT_(ftp_skip_ip);      /* skip the IP address the FTP server passes on to
                            us */
  _BIT_(wildcard_enabled); /* enable wildcard matching */
#endif
  _BIT_(http_fail_on_error);  /* fail on HTTP error codes >= 400 */
  _BIT_(http_keep_sending_on_error); /* for HTTP status codes >= 300 */
  _BIT_(http_transfer_encoding); /* request compressed HTTP transfer-encoding */
  _BIT_(allow_auth_to_other_hosts);
  _BIT_(include_header); /* include received protocol headers in data output */
  _BIT_(http_set_referer); /* is a custom referer used */
  _BIT_(http_auto_referer); /* set "correct" referer when following
                             location: */
  _BIT_(opt_no_body);    /* as set with CURLOPT_NOBODY */
  _BIT_(verbose);        /* output verbosity */
  _BIT_(reuse_forbid);   /* forbidden to be reused, close after use */
  _BIT_(reuse_fresh);    /* do not reuse an existing connection  */
  _BIT_(no_signal);      /* do not use any signal/alarm handler */
  _BIT_(tcp_nodelay);    /* whether to enable TCP_NODELAY or not */
  _BIT_(ignorecl);       /* ignore content length */
  _BIT_(http_te_skip);   /* pass the raw body data to the user, even when
                          transfer-encoded (chunked, compressed) */
  _BIT_(http_ce_skip);   /* pass the raw body data to the user, even when
                          content-encoded (chunked, compressed) */
  _BIT_(proxy_transfer_mode); /* set transfer mode (;type=<a|i>) when doing
                               FTP via an HTTP proxy */
#if defined(HAVE_GSSAPI) || defined(USE_WINDOWS_SSPI)
  _BIT_(socks5_gssapi_nec); /* Flag to support NEC SOCKS5 server */
#endif
  _BIT_(sasl_ir);         /* Enable/disable SASL initial response */
  _BIT_(tcp_keepalive);  /* use TCP keepalives */
  _BIT_(tcp_fastopen);   /* use TCP Fast Open */
  _BIT_(ssl_enable_alpn);/* TLS ALPN extension? */
  _BIT_(path_as_is);     /* allow dotdots? */
  _BIT_(pipewait);       /* wait for multiplex status before starting a new
                          connection */
  _BIT_(suppress_connect_headers); /* suppress proxy CONNECT response headers
                                    from user callbacks */
  _BIT_(dns_shuffle_addresses); /* whether to shuffle addresses before use */
#ifndef CURL_DISABLE_PROXY
  _BIT_(haproxyprotocol); /* whether to send HAProxy PROXY protocol v1
                           header */
#endif
#ifdef USE_UNIX_SOCKETS
  _BIT_(abstract_unix_socket);
#endif
  _BIT_(disallow_username_in_url); /* disallow username in URL */
#ifndef CURL_DISABLE_DOH
  _BIT_(doh); /* DNS-over-HTTPS enabled */
  _BIT_(doh_verifypeer);     /* DoH certificate peer verification */
  _BIT_(doh_verifyhost);     /* DoH certificate hostname verification */
  _BIT_(doh_verifystatus);   /* DoH certificate status verification */
#endif
  _BIT_(http09_allowed); /* allow HTTP/0.9 responses */
#ifndef CURL_DISABLE_WEBSOCKETS
  _BIT_(ws_raw_mode);
  _BIT_(ws_no_auto_pong);
#endif
};
